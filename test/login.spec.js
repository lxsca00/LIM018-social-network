...
describe('fEventLogin funciona correctamente', () => {
  ...
  it('debería aparecer error cuando no hay un usuario registrado', (done) => {
    ...
    btnLogin.click();
    setTimeout(() => {
      expect(document.getElementById('login-error-modal').innerHTML).toBe('No existe ningún usuario registrado con este email.');
      done();
    });
  });
  ...
  it('debería aparecer error cuando hay un email en uso', (done) => {
    ...
    btnLogin.click();
    setTimeout(() => {
      expect(document.getElementById('login-error-modal').innerHTML).toBe('Proporcione una dirección de correo válida.');
      done();
    });
  });
  ...
  it('debería aparecer error cuando no se ingresa contraseña', (done) => {
    ...
    btnLogin.click();
    setTimeout(() => {
      expect(document.getElementById('login-error-modal').innerHTML).toBe('El ingreso de contraseña es obligatorio.');
      done();
    });
  });
  ...
  it('debería aparecer error cuando ingresa contraseña menor a 6 dígitos', (done) => {
    ...
    btnLogin.click();
    setTimeout(() => {
      expect(document.getElementById('login-error-modal').innerHTML).toBe('La contraseña ingresada es incorrecta.');
      done();
    });
  });
  ...
  it('debería aparecer error', (done) => {
    ...
    btnLogin.click();
    setTimeout(() => {
      expect(document.getElementById('login-error-modal').innerHTML).toBe('Por favor vuelve a intentarlo.');
      done();
    });
  });
});
